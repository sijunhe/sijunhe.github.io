...
From the plot above on the top (from the original Switch Transformers paper), you can see how scaling T5-base to 16, 32, 64, and 128 experts leads to corresponding performance gains. My experiments, shown in the plot above on the bottom, tell the same story – the loss curves get progressively lower as we add more experts to GPT2-small. While the metrics differ between the two plots (negative log perplexity vs. raw loss), the trend is identical: more experts yield better performance at the same computational cost.

## 2. Compared with dense model, you can achieve same level performance with less model FLOPS.

## 3. Under the same amount of parameters, MoE models underperforms Dense Model significantly.
...
以GPT-2（124M）模型为基础，我们对其FFN层进行了MoE（Mixture of Experts）化处理。随着专家数量（2, 4, 8, 16）的逐步增加，模型的收敛曲线显示出模型效果显著提升，趋势符合scaling law
结论 2：相比 Dense 模型，MoE能用更小的 FLOPS 数达到相同的模型效果
* 随着专家数增多，MoE 模型的效果逐渐接近 GPT-2-medium (353M), 按照当前的结论外推，switch transformers 中说的 64 experts 能够打平3 倍大的 dense 完全可信
结论 3：在相同参数量下，MoE 模型弱于 Dense 模型
switch transformers 中一直在谈模型 的FLOPS，基本对应模型推理成本，但是闭口不谈模型参数量，其实很鸡贼。
其实，同等参数量下，Dense 模型要比 MoE 强不少。




到底多大的 MoE 模型才能和 Dense 模型效果持平?
Switch Transformers 结论: 64 experts 的 MoE T5-Base 略强于和 T5-Large （3.5X FLOPS vs. T5-Base）

迁移到 GPT2结构上结论应该也是 hold 的。那么 64 experts 的 GPT2 有多大呢?
model
num of params
GPT-2
124M
...
GPT-2 MoE - 64 expert
3.6B
也就是说，对于 MoE 来说，需要 32 expert 或者 64 expert 才能和 GPT-2 medium 效果对齐. 即 MoE 1.8B - 3.6B  vs. Dense 353M. 
如果从总参数量的角度叙事，瞬间 MoE 就没那么有吸引力了，远不如从 FLOPS 角度的故事漂亮
